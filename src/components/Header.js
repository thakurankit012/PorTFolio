    ...
    <header className='flex justify-between px-10 py-2 bg-primary text-primary'>
      ...
      <nav className='hidden md:block'>
        <ul className='flex'>
          ...
          <li>
            <button onClick={() => changeLanguage('es')} className='mr-2'>Spanish</button>
          </li>
      ...
      <nav className={!toggle ? 'mobile-nav left-[-100%]' : 'mobile-nav left-0'}>
        <ul className='flex flex-col py-10 text-5xl'>
          ...
          {/* Language Selection */}
          <li>
            <a onClick={() => changeLanguage('en')} className='mr-2'>English</a>
          </li>
          <li>
            <a onClick={() => changeLanguage('es')} className='mr-2'>Spanish</a>
          </li>
          {/* Add more buttons for other languages as needed */}
        </ul>
      </nav>
      ...
      <div className='md:hidden'>
        ...
        <div className={`absolute top-14 right-4 z-20 bg-primary rounded-lg shadow-md py-2 px-4 ${toggle ? 'block' : 'hidden'}`}>
          <button onClick={() => changeLanguage('en')} className='block w-full py-2 text-left'>English</button>
          <button onClick={() => changeLanguage('es')} className='block w-full py-2 text-left'>Spanish</button>
          {/* Add more buttons for other languages as needed */}
        </div>
      </div>
    </header>
